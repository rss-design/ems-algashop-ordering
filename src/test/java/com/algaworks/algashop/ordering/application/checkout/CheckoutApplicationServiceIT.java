...
class CheckoutApplicationServiceIT {
    ...
    @BeforeEach
    public void setup() {
        Mockito.when(shippingCostService.calculate(
                Mockito.any(ShippingCostService.CalculationRequest.class)))
    ...
    void shouldCheckout() {
        ...
        Assertions.assertThat(createdOrder.get().totalAmount().value())
    ...
    void shouldThrowShoppingCartCantProceedToCheckoutExceptionWhenCartContainsUnavailableItems() {
        ...
        Product unavailableProduct =
            ProductTestDataBuilder.aProduct().id(product.id()).inStock(false).build();
